	...
	function($) {
		...
		$( document ).on(
			...
			function(event){
				...
				var widget = $( this ).closest( '.wp4t-absence-widget' );
				...
				wp4tAbsenceApiCall( widget, { operation: 'add', ID: parseInt( widget.attr( 'data-user-id' ), 10 ), until: '' }, 'Absence added for this meeting.', function(){


					widget.attr( 'data-absent', '1' );


					widget.find( '.wp4t-absence-until' ).show();


					widget.find( '.wp4t-absence-until-select' ).trigger( 'focus' );


					setAbsenceButtonState( widget, true );
		...
		$( document ).on(
			...
			function(){
				...
				if (!widget.length || widget.attr( 'data-absent' ) !== '1') {
				...
				wp4tAbsenceApiCall( widget, { operation: 'add', ID: parseInt( widget.attr( 'data-user-id' ), 10 ), until: until ? until : '' }, 'Absence extended.' );


				widget.attr( 'data-until', until ? until : '' );
		...
		$( document ).on(
			...
			function(event){
				...
				var until = widget.attr( 'data-until' );


				wp4tAbsenceApiCall( widget, { operation: 'remove', ID: parseInt( widget.attr( 'data-user-id' ), 10 ), until: until ? until : '' }, 'Absence removed.', function(){


					widget.attr( 'data-absent', '0' );


					widget.attr( 'data-until', '' );


					widget.find( '.wp4t-absence-until-select' ).val( '' );


					widget.find( '.wp4t-absence-until' ).hide();


					setAbsenceButtonState( widget, false );
		...
		function setAbsenceButtonState( widget, isAbsent ) {


			var line = widget.children( 'p' ).first();
			...
			if (isAbsent) {
				...
				line.html( '<button type="button" class="agenda-tooltip wp4t-absence-remove" aria-label="Remove me from absences"><span class="agenda-tooltip-text">Remove Me</span><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24" aria-hidden="true"><path d="M12 4C7.6 4 4 7.6 4 12s3.6 8 8 8 8-3.6 8-8-3.6-8-8-8zm3.5 10.1-1.4 1.4-2.1-2.1-2.1 2.1-1.4-1.4 2.1-2.1-2.1-2.1 1.4-1.4 2.1 2.1 2.1-2.1 1.4 1.4-2.1 2.1 2.1 2.1z"></path></svg></button> Remove me from absences' );


			} else {


				line.html( '<button type="button" class="agenda-tooltip wp4t-absence-add" aria-label="Add me to absences"><span class="agenda-tooltip-text">Add Me</span><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24" aria-hidden="true"><path d="M12 4C7.6 4 4 7.6 4 12s3.6 8 8 8 8-3.6 8-8-3.6-8-8-8zm1 4.5v2.5h2.5v2H13v2.5h-2V13H8.5v-2H11V8.5h2z"></path></svg></button> Add me to absences' );
		...
		function wp4tAbsenceApiCall( widget, payload, successMessage, onSuccess ) {


			var endpoint = widget.attr( 'data-endpoint' );


			var post_id = parseInt( widget.attr( 'data-post-id' ), 10 );


			var status = widget.find( '.wp4t-absence-status' );


			if (!endpoint || !post_id) {


				status.html( 'Could not determine absences endpoint.' );
			...
			if (typeof rsvpmaker_rest !== 'undefined' && rsvpmaker_rest.nonce) {


				headers['X-WP-Nonce'] = rsvpmaker_rest.nonce;
			...
			$.ajax(
...

				{
					...
					url: endpoint + '?post_id=' + post_id + '&_locale=user',
					...
					error: function(xhr) {
						...
						if (xhr && xhr.responseJSON && xhr.responseJSON.message) {
							...
							status.html( 'Could not update absences. Please try again.' );
