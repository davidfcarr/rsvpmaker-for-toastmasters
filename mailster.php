...
if(!$whitelist) {
    $success = $wpdb->insert($wpdb->prefix.'mailster_groups',array('name'=>'whitelist','is_core_group'=> 0, 'core_group_id' => 0 ));
    if($success)
...
array_shift($urlparts);array_shift($urlparts); //get rid of protocol
if($urlparts[0] == 'www')
...
if(!$member_list)
{
    ...
    $memberarray = array('asset_id' => '0','name' => $name,'admin_mail' => get_bloginfo('admin_email'),'published' => NULL,'active' => '0','public_registration' => '1','sending_public' => '0','sending_recipients' => '1','sending_admin' => '1','sending_group' => '1','sending_group_id' => $whitelist,'mod_mode' => '0','mod_moderated_group' => '0','mod_approve_recipients' => '0','mod_approve_group' => '0','mod_approve_group_id' => '0','mod_info_sender_moderation' => '0','mod_info_sender_approval' => '0','mod_info_sender_rejection' => '0','disable_mail_footer' => '0','allow_subscribe' => '1','allow_unsubscribe' => '1','reply_to_sender' => '2','list_mail' => $list_email,'subject_prefix' => '','use_cms_mailer' => '0','copy_to_sender' => '1','mail_in_user' => $list_email,'mail_in_pw' => $password,'mail_out_user' => $list_email,'mail_out_pw' => $password,'server_inb_id' => $pop3,'server_out_id' => $smtp,'custom_header_plain' => 'Message from {name}  {email} ({date}):','custom_footer_plain' => 'From {site} 
    Unsubscribe: '.$unsubscribe_url.'{email}','custom_header_html' => 'Message From {name} <a href="mailto:{email}">Reply to {email}</a> ({date}):<br /><br />','custom_footer_html' => sprintf('From {site} <a href="%s{email}">Unsubscribe</a>',$unsubscribe_url),'mail_format_conv' => '0','mail_format_altbody' => '1','alibi_to_mail' => NULL,'addressing_mode' => '1','mail_from_mode' => '2','name_from_mode' => '2','archive_mode' => '0','archive_retention' => '0','archive2article' => '0','archive2article_author' => '0','archive2article_cat' => '0','archive2article_state' => '1','archive_offline' => '0','bounce_mode' => '0','bounce_mail' => '','bcc_count' => '10','incl_orig_headers' => '0','max_send_attempts' => '5','filter_mails' => '0','allow_bulk_precedence' => '0','clean_up_subject' => '1','cstate' => '1','mail_size_limit' => '0','notify_not_fwd_sender' => '1','save_send_reports' => '7','subscribe_mode' => '0','unsubscribe_mode' => '0','welcome_msg' => '1','welcome_msg_admin' => '0','goodbye_msg' => '1','goodbye_msg_admin' => '0','allow_digests' => '0','front_archive_access' => '0','mail_content' => NULL);
...
if(!$officer_list)
{
    ...
    $password = wp_generate_password();
    $unsubscribe_url = site_url('?rsvpmail_unsubscribe=');
    $memberarray = array('asset_id' => '0','name' => $name,'admin_mail' => get_bloginfo('admin_email'),'published' => NULL,'active' => '0','public_registration' => '1','sending_public' => '0','sending_recipients' => '1','sending_admin' => '1','sending_group' => '1','sending_group_id' => $whitelist,'mod_mode' => '0','mod_moderated_group' => '0','mod_approve_recipients' => '0','mod_approve_group' => '0','mod_approve_group_id' => '0','mod_info_sender_moderation' => '0','mod_info_sender_approval' => '0','mod_info_sender_rejection' => '0','disable_mail_footer' => '0','allow_subscribe' => '1','allow_unsubscribe' => '1','reply_to_sender' => '2','list_mail' => $officer_list_email,'subject_prefix' => '','use_cms_mailer' => '0','copy_to_sender' => '1','mail_in_user' => $officer_list_email,'mail_in_pw' => $password,'mail_out_user' => $officer_list_email,'mail_out_pw' => $password,'server_inb_id' => $pop3,'server_out_id' => $smtp,'custom_header_plain' => 'Message from {name}  {email} ({date}):','custom_footer_plain' => 'From {site} 
    Unsubscribe: '.$unsubscribe_url.'{email}','custom_header_html' => 'Message From {name} <a href="mailto:{email}">Reply to {email}</a> ({date}):<br /><br />','custom_footer_html' => sprintf('From {site} <a href="%s{email}">Unsubscribe</a>',$unsubscribe_url),'mail_format_conv' => '0','mail_format_altbody' => '1','alibi_to_mail' => NULL,'addressing_mode' => '1','mail_from_mode' => '2','name_from_mode' => '2','archive_mode' => '0','archive_retention' => '0','archive2article' => '0','archive2article_author' => '0','archive2article_cat' => '0','archive2article_state' => '1','archive_offline' => '0','bounce_mode' => '0','bounce_mail' => '','bcc_count' => '10','incl_orig_headers' => '0','max_send_attempts' => '5','filter_mails' => '0','allow_bulk_precedence' => '0','clean_up_subject' => '1','cstate' => '1','mail_size_limit' => '0','notify_not_fwd_sender' => '1','save_send_reports' => '7','subscribe_mode' => '0','unsubscribe_mode' => '0','welcome_msg' => '1','welcome_msg_admin' => '0','goodbye_msg' => '1','goodbye_msg_admin' => '0','allow_digests' => '0','front_archive_access' => '0','mail_content' => NULL);
...
if(!$onlist && !in_array(strtolower($user->user_email),$unsub))
    {
        $wpdb->insert($wpdb->prefix."mailster_list_members",array('is_core_user' => 1, 'user_id' => $user->ID, 'list_id' => $member_list ));
    }
}
...
foreach($officers as $officer)
    {
        ...
        if(!$onlist && !in_array(strtolower($user->user_email),$unsub))
            {
                $wpdb->insert($wpdb->prefix."mailster_list_members",array('is_core_user' => 1, 'user_id' => $user->ID, 'list_id' => $officer_list ));
            }        
    }
...
if(!empty($add_to_whitelist) && is_array($add_to_whitelist))
    {
        ...
        {
            $wpdb->insert($wpdb->prefix.'mailster_users', array('name' => $email, 'email' => $email ) );
            $subscriber_id = $wpdb->insert_id;
            $wpdb->insert($wpdb->prefix.'mailster_group_users',array('is_core_user' => 0, 'user_id' => $subscriber_id, 'group_id' => $whitelist));
...
if(isset($_POST['extra'])) {
    ...
    foreach($extra as $email) {
        ...
        if(in_array($email,$unsub))
            ...
            {
                $wpdb->insert($wpdb->prefix."mailster_list_members",array('is_core_user' => 0, 'user_id' => $subscriber_id, 'list_id' => $member_list ));
