	...
	foreach ( $assignments as $field => $values ) {
		...
		if ( is_club_member() ) {
			...
			if ( is_edit_roles() && ( current_user_can( 'edit_signups' ) || edit_signups_role() ) ) {
				...
				if ( strpos( $field, 'Speaker' ) ) {
					$output .= sprintf(
						'<div class="update_form" id="update' . $field . '">%s
		...
		if ( is_club_member() && ! ( isset( $_REQUEST['edit_roles'] ) || isset( $_REQUEST['recommend_roles'] ) || ( isset( $_REQUEST['page'] ) && ( $_REQUEST['page'] == 'toastmasters_reconcile' ) ) ) ) {
			...
			if ( is_single() && current_user_can( 'edit_signups' ) ) {
				...
				$output .= sprintf( '<div id="editonewrapper%s""><a class="editonelink" editone="%s">Edit</a></div><form id="editone%s" method="post" class="edit_one_form" action="%s" style="display: block;"><input type="hidden" name="post_id" value="%d"><input type="hidden" name="role" value="%s"><div>%s</div>', $field, $field, $field, $permalink, $post->ID, $field, $editone ) . '<button name="edit_one" id="edit_one_button' . esc_attr($field) . '" value="1">' . __( 'Submit', 'rsvpmaker-for-toastmasters' ) . '</button>'.rsvpmaker_nonce('return').'</form>';
			}
		}
...
function wp4t_set_status_form() {
	...
	if ( isset( $_POST['month'][0] ) && is_club_member() && wp_verify_nonce(rsvpmaker_nonce_data('data'),rsvpmaker_nonce_data('key')) ) {
		$exp       = sanitize_text_field($_POST['year'][0] . '-' . $_POST['month'][0] . '-' . $_POST['day'][0] . ' ' . $_POST['hour'][0] . ':00:00');
		$member_id = (int) $_POST['member_id'];
		wp4t_set_member_status( $member_id, sanitize_text_field($_POST['status']), $exp );
	}
	if ( isset( $_POST['remove_status'] ) && wp_verify_nonce(rsvpmaker_nonce_data('data'),rsvpmaker_nonce_data('key')) ) {
...
<select id="month<?php echo esc_attr($index); ?>" name="month[<?php echo esc_attr($index); ?>]"> 
	<?php
	for ( $i = 1; $i <= 12; $i++ ) {
		echo '<option ';
		if ( $i == $month ) {
...
<select  id="day<?php echo esc_attr($index); ?>"  name="day[<?php echo esc_attr($index); ?>]"> 
	<?php
	if ( $day == 0 ) {
...
<select  id="year<?php echo esc_attr($index); ?>" name="year[<?php echo esc_attr($index); ?>]"> 
	<?php
	$y     = (int) date( 'Y' );
	$limit = $y + 3;
	for ( $i = $y; $i < $limit; $i++ ) {
...
<input type="hidden" id="datepicker<?php echo esc_attr($index); ?>" value="<?php echo esc_attr($jquery_date); ?>">
	<?php echo __( 'Hour:', 'rsvpmaker' ); ?> <select name="hour[<?php echo esc_attr($index); ?>]"> 
	<?php
	for ( $i = 0; $i < 24; $i++ ) {
...
<input type="submit" name="submit" value="<?php _e( 'Submit', 'rsvpmaker-for-toastmasters' ); ?>" />
<?php rsvpmaker_nonce(); ?>
</form></p>
	<?php
	printf( '<form action="' . admin_url( 'profile.php?page=wp4t_set_status_form' ) . '" method="post">%s<input type="submit" name="remove_status" value="' . __( 'Clear Status', 'rsvpmaker-for-toastmasters' ) . '" />'.rsvpmaker_nonce('return').'</form>', $dropdown );
...
function extended_list() {
	...
	if ( ! empty( $_POST['guestlist'] ) ) {
		foreach ( $_POST['guestlist'] as $email ) {
			add_to_mailman( sanitize_text_field($email), 'g' );
		}
...
<h3>
	<?php
	if ( isset( $userdata ) ) {
		_e( 'Edit Entry', 'rsvpmaker-for-toastmasters' );
	} else {
		_e( 'Add Entry', 'rsvpmaker-for-toastmasters' );
	}
	?>
</h3>
<table>
<tr><td><?php _e( 'First', 'rsvpmaker-for-toastmasters' ); ?></td><td><input name="user[first_name]" value="
				  <?php
					if ( isset( $userdata['first_name'] ) ) {
						echo esc_attr($userdata['first_name']);}
					?>
	" ></td></tr>
<tr><td><?php _e( 'Last', 'rsvpmaker-for-toastmasters' ); ?></td><td><input name="user[last_name]"  value="
				  <?php
					if ( isset( $userdata['last_name'] ) ) {
						echo esc_attr($userdata['last_name']);}
					?>
	" ></td></tr
><tr><td><?php _e( 'Email', 'rsvpmaker-for-toastmasters' ); ?></td><td><input name="user[user_email]"  value="
				   <?php
					if ( isset( $userdata['user_email'] ) ) {
						echo esc_attr($userdata['user_email']);}
					?>
	" ></td></tr>
<tr><td><?php _e( 'Home', 'rsvpmaker-for-toastmasters' ); ?></td><td><input name="user[home_phone]"  value="
				  <?php
					if ( isset( $userdata['home_phone'] ) ) {
						echo esc_attr($userdata['home_phone']);}
					?>
	" ></td></tr>
<tr><td><?php _e( 'Work', 'rsvpmaker-for-toastmasters' ); ?></td><td><input name="user[work_phone]"  value="
				  <?php
					if ( isset( $userdata['work_phone'] ) ) {
						echo esc_attr($userdata['work_phone']);}
					?>
	" ></td></tr>
<tr><td><?php _e( 'Mobile', 'rsvpmaker-for-toastmasters' ); ?></td><td><input name="user[mobile_phone]"  value="
				  <?php
					if ( isset( $userdata['mobile_phone'] ) ) {
						echo esc_attr($userdata['mobile_phone']);}
					?>
	" ></td></tr>
<tr><td><?php _e( 'Note', 'rsvpmaker-for-toastmasters' ); ?></td><td><textarea name="user[note]" rows="5" cols="80">
				  <?php
					if ( isset( $userdata['note'] ) ) {
						echo esc_attr($userdata['note']);}
					?>
	</textarea></td></tr>
</table>
<br /><input type="radio" name="guest" value="1" 
	<?php
	if ( ! isset( $userdata['ID'] ) ) {
		echo 'checked="checked"';}
	?>
	> <?php _e( 'Guest', 'rsvpmaker-for-toastmasters' ); ?>
<br /><input type="radio" name="guest" value="1" 
	<?php
	if ( isset( $userdata['ID'] ) ) {
		echo 'checked="checked"';}
	?>
	> <?php _e( 'Former Member', 'rsvpmaker-for-toastmasters' ); ?>
<input type="hidden" name="lookup" value="
	<?php
	if ( isset( $row->sort ) ) {
		echo esc_attr($row->sort);}
	?>
	" />
...
<p>(See <a href="http://wp4toastmasters.com/2016/11/28/email-list-integration-for-your-toastmasters-club/" target="_blank"><?php _e( 'Documentation', 'rsvpmaker-for-toastmasters' ); ?></a>)</p>
				<p><?php _e( 'List email address', 'rsvpmaker-for-toastmasters' ); ?>: <input type="text" name="wp4toastmasters_mailman[members]" value="
							 <?php
								if ( isset( $wp4toastmasters_mailman['members'] ) ) {
									echo esc_attr($wp4toastmasters_mailman['members']);}
								?>
	" /></p>
<p><?php _e( 'Path', 'rsvpmaker-for-toastmasters' ); ?>: <input type="text" name="wp4toastmasters_mailman[mpath]" value="
				<?php
				if ( isset( $wp4toastmasters_mailman['mpath'] ) ) {
					echo esc_attr($wp4toastmasters_mailman['mpath']);}
				?>
	" /> <?php _e( 'Password', 'rsvpmaker-for-toastmasters' ); ?>: <input type="text" name="wp4toastmasters_mailman[mpass]" value="
				<?php
				if ( isset( $wp4toastmasters_mailman['mpass'] ) ) {
					echo esc_attr($wp4toastmasters_mailman['mpass']);}
				?>
	" /></p>
				<?php
				if ( isset( $wp4toastmasters_mailman['mpass'] ) ) {
...
<p><?php _e( 'List email address', 'rsvpmaker-for-toastmasters' ); ?>: <input type="text" name="wp4toastmasters_mailman[officers]" value="
				<?php
				if ( isset( $wp4toastmasters_mailman['officers'] ) ) {
					echo esc_attr($wp4toastmasters_mailman['officers']);}
				?>
	" /></p>
<p><?php _e( 'Path', 'rsvpmaker-for-toastmasters' ); ?>: <input type="text" name="wp4toastmasters_mailman[opath]" value="
				<?php
				if ( isset( $wp4toastmasters_mailman['opath'] ) ) {
					echo esc_attr($wp4toastmasters_mailman['opath']);}
				?>
	" /> <?php _e( 'Password', 'rsvpmaker-for-toastmasters' ); ?>: <input type="text" name="wp4toastmasters_mailman[opass]" value="
				<?php
				if ( isset( $wp4toastmasters_mailman['opass'] ) ) {
					echo esc_attr($wp4toastmasters_mailman['opass']);}
				?>
	" />
...
<p><?php _e( 'List email address', 'rsvpmaker-for-toastmasters' ); ?>: <input type="text" name="wp4toastmasters_mailman[guest]" value="
				<?php
				if ( isset( $wp4toastmasters_mailman['guest'] ) ) {
					echo esc_attr($wp4toastmasters_mailman['guest']);}
				?>
	" /></p>
<p><?php _e( 'Path', 'rsvpmaker-for-toastmasters' ); ?>: <input type="text" name="wp4toastmasters_mailman[gpath]" value="
				<?php
				if ( isset( $wp4toastmasters_mailman['gpath'] ) ) {
					echo esc_attr($wp4toastmasters_mailman['gpath']);}
				?>
	" /> <?php _e( 'Password', 'rsvpmaker-for-toastmasters' ); ?>: <input type="text" name="wp4toastmasters_mailman[gpass]" value="
				<?php
				if ( isset( $wp4toastmasters_mailman['gpass'] ) ) {
					echo esc_attr($wp4toastmasters_mailman['gpass']);}
				?>
	" />
...
<p><?php _e( 'Agenda Layout', 'rsvpmaker-for-toastmasters' ); ?> 
	...
	if ( $layout_mod < '2021-05-05' ) {
		echo ' <input type="checkbox" name="set_layout_default" value="' . $layout_id . '"> ' . __( 'Update to current default? (will overwrite customizations)', 'rsvpmaker-for-toastmasters' );
	}
...
function wp4toast_reminders_cron_status() {
	...
	$output .= sprintf( '<p>Next meeting: %s</p>', rsvpmaker_date( $rsvp_options['long_date'] . ' ' . $rsvp_options['time_format'], rsvpmaker_strtotime( $post->datetime ) ) );
...
function wpt_custom_layout_default($sidebar_officers = false) {
	global $wpdb;
	$sql = "SELECT meta_value FROM $wpdb->postmeta WHERE meta_key='_tm_sidebar' ORDER BY meta_id DESC";
	$sidebar = $wpdb->get_var($sql);
	if(!$sidebar_officers) {
...
function list_mailman_pending( $list = 'm' ) {
		...
		if ( isset( $_POST['always_approve'] ) && wp_verify_nonce(rsvpmaker_nonce_data('data'),rsvpmaker_nonce_data('key')) ) {
			$adminurl = $url;
			foreach ( $_POST['always_approve'] as $email ) {
				$emailcoded = str_replace( '@', '%40', sanitize_text_field($email) );
		...
		if ( isset( $_POST['approve'] ) && wp_verify_nonce(rsvpmaker_nonce_data('data'),rsvpmaker_nonce_data('key')) ) {
			$adminurl = $url;
			foreach ( $_POST['approve'] as $email ) {
				$emailcoded = str_replace( '@', '%40', sanitize_text_field($email) );
				foreach ( $_POST['msgid'][ $email ] as $msgid ) {
					$adminurl .= '&' . $emailcoded . '=' . sanitize_text_field($msgid);
				}
		...
		if ( isset( $_POST['deny'] ) && wp_verify_nonce(rsvpmaker_nonce_data('data'),rsvpmaker_nonce_data('key')) ) {
			$adminurl = $url;
...
function mailman() {
	...
	$mm      = get_option( 'wp4toastmasters_mailman' );
	$list    = isset( $_REQUEST['list'] ) ? sanitize_text_field($_REQUEST['list']) : '';
	$lists   = array(
		'm' => 'members',
		'o' => 'officers',
		'g' => 'guests',
	...
	printf( '<form action="%s" method="get"><input type="hidden" name="page" value="mailman" />Select List: <select name="list">%s</select><button>Go</button>%s</form>', admin_url( 'users.php' ), $options, rsvpmaker_nonce('return') );

	if ( isset( $_REQUEST['unsubscribe'] ) ) {
		unsubscribe_mailman_by_email( sanitize_text_field($_REQUEST['unsubscribe']), sanitize_text_field($_REQUEST['list']) );
	}
	if ( isset( $_POST['add_email'] ) && isset( $_POST['list'] ) && wp_verify_nonce(rsvpmaker_nonce_data('data'),rsvpmaker_nonce_data('key')) ) {
		preg_match_all( '/[a-z\d._%+-]+@[a-z\d.-]+\.[a-z]{2,4}\b/i', sanitize_text_field($_POST['add_email']), $matches );
		if ( ! empty( $matches[0] ) ) {
			foreach ( $matches[0] as $email ) {
				add_to_mailman( $email, sanitize_text_field($_POST['list']) );
			}
		}
	}
	...
	if ( ! empty( $mm[ $list . 'pass' ] ) ) {
		...
		printf( '<h3>%s</h3><p>%s</p><form action="%s" method="post"><input type="hidden" name="list" value="%s" /><textarea name="add_email"  rows="5" cols="80">%s</textarea><br /><button>Add</button>%s</form>', __( 'Add Email', 'rsvpmaker-for-toastmasters' ), __( 'Add one or more email addresses separated by commas or line breaks', 'rsvpmaker-for-toastmasters' ), admin_url( 'users.php?page=mailman' ), $list, implode( "\n", $suggestions ), rsvpmaker_nonce('return') );
		if ( $list == 'g' ) {
		...
		if ( $frontend ) {
			...
			if ( $events ) {
				foreach ( $events as $event ) {
					$link .= '<li><a href="' . rsvpmaker_permalink_query( $event->ID ) . '"' . $blank . '>' . rsvpmaker_date( $rsvp_options['short_date'], strtotime( $event->datetime ) ) . '</a></li>';
					if ( current_user_can( $security['edit_signups'] ) || edit_signups_role( $event->ID ) ) {
						$link .= '<li><a href="' . rsvpmaker_permalink_query( $event->ID, 'edit_roles=1' ) . '"' . $blank . '>' . __( 'Edit', 'rsvpmaker-for-toastmasters' ) . ' ' . rsvpmaker_date( $rsvp_options['short_date'], rsvpmaker_strtotime( $event->datetime ) ) . '</a></li>';
					}
				}
			}
...
function wpt_member_upload_to_array() {
	...
	if ( ! empty( $_FILES['upload_file']['tmp_name'] ) ) {
		$file = fopen( stripslashes($_FILES['upload_file']['tmp_name']), 'r' );
		if ( $file ) {
				...
				if ( $dates ) {
					foreach ( $dates as $row ) {
						...
						if ( isset( $ev[ $row->postID ] ) ) {
							$ev[ $row->postID ] .= ', ' . rsvpmaker_date( $dateformat, strtotime( $row->datetime ) );
						} else {
...
function toolbar_link_to_agenda( $wp_admin_bar ) {
	...
	$link = admin_url('post.php?post='.$layout_id.'&action=edit');
	$args  = array(
		'id'    => 'agenda_layout',
		'title' => 'Edit Agenda Layout',
		'href'  => $link,
...
function tmlayout_meeting_date( $atts ) {
	...
	if ( ! empty( $datestring ) ) {
		return date( $rsvp_options['long_date'], strtotime( $datestring ) );
	}
}
			...
			if ( $results ) {
				foreach ( $results as $row ) {
					...
					if ( ! empty( $roledata['title'] ) ) {
						$speech_details .= '<br />' . esc_html($roledata['title']);
					}
...
function tm_absence( $atts ) {
	...
	global $rsvp_options;
	$output = '';
	if ( isset( $_POST['absences'] ) && wp_verify_nonce(rsvpmaker_nonce_data('data'),rsvpmaker_nonce_data('key')) ) {
		foreach ( $_POST['absences'] as $a ) {
			if ( ! empty( $a ) ) {
				add_post_meta( $post->ID, 'tm_absence', sanitize_text_field($a) );
			}
		}
	}
	if ( isset( $_POST['cancel_absences'] ) && wp_verify_nonce(rsvpmaker_nonce_data('data'),rsvpmaker_nonce_data('key')) ) {
	...
	if ( ! empty( $absences ) && is_array( $absences ) && in_array( $current_user->ID, $absences ) ) {
		$output .= sprintf( '<form method="post" action="%s"><input type="hidden" name="cancel_absence" value="1"><button>Cancel My Absence </button>%s</form>', get_permalink(),rsvpmaker_nonce('return') );
	} else {
		$output .= sprintf( '<form class="planned_absence_form" method="post" action="%s"><input type="hidden" name="add_absence" value="1"><button>Planned Absence </button>%s</form>', get_permalink(), rsvpmaker_nonce('return') );
	}
