...
class Toast_Norole_Controller extends WP_REST_Controller {
	public function register_routes() {
		$namespace = 'rsvptm/v1';
		$path      = 'wp4t_norole/(?P<post_id>[0-9]+)';
		register_rest_route(
			$namespace,
			'/' . $path,
			array(
	...
	rsvpmaker_debug_log($_SERVER['SERVER_NAME'].' '.$_SERVER['REQUEST_URI'],'rsvpmaker_api');
		...
		$wp4t_norole = wpt_norole($post_id);
		return new WP_REST_Response( $wp4t_norole, 200 );
	}
}
...
class Toast_Agenda_Timing extends WP_REST_Controller {
	...
	public function get_items( $request ) {
	rsvpmaker_debug_log($_SERVER['SERVER_NAME'].' '.$_SERVER['REQUEST_URI'],'rsvpmaker_api');
		$timing = wp4t_get_agenda_timing( $request['post_id'] );
...
class Toast_Manual_Lookup extends WP_REST_Controller {
	...
	rsvpmaker_debug_log($_SERVER['SERVER_NAME'].' '.$_SERVER['REQUEST_URI'],'rsvpmaker_api');
		...
		$options  = wp4t_get_manuals_by_type_options( $type );
		$projects = '';
...
class Editor_Assign extends WP_REST_Controller {
	...
	rsvpmaker_debug_log($_SERVER['SERVER_NAME'].' '.$_SERVER['REQUEST_URI'],'rsvpmaker_api');
		...
		$log    = wp4t_get_member_name( $editor_id ) . ' assigned ' . wp4t_clean_role( $role ) . ' to ' . wp4t_get_member_name( $user_id ) . ' for ' . $timestamp;
		if ( $was ) {
		...
		if ( strpos( $role, 'peaker' ) ) {
			$track  = wp4t_get_speaking_track( $user_id );
			$type   = $track['type'];
			$manual = $track['manual'];
...
function wpt_api_add_absence() {
	...
	return wp4t_get_member_name($away_user_id)." added to absences.";
}
...
class WPTM_Reports extends WP_REST_Controller {
	...
	rsvpmaker_debug_log($_SERVER['SERVER_NAME'].' '.$_SERVER['REQUEST_URI'],'rsvpmaker_api');
		...
		$content = '';
		if ( $report == 'wp4t_speeches_by_manual' ) {
			$content = wp4t_speeches_by_manual( $user_id );
...
<!-- /wp:paragraph -->',admin_url('edit.php?post_type=tmminutes&page=toastmasters_member_votes'))
		...
		$agenda_memberlist = get_transient('wp4t_memberlist');
		$votingdata['agenda_memberlist'] = $agenda_memberlist;
		if($agenda_memberlist && is_array($agenda_memberlist)) {
			//normally will be set by previous agenda screen lookup
			$votingdata['cached_memberlist'] = true;
			foreach($agenda_memberlist as $member) {
				$memberlist[] = array('label'=>$member['name'],'value'=>$member['name']);
			}
		}
		else {
			$votingdata['cached_memberlist'] = false;
			$memberlist = [];
			...
			if(!empty($post_id)) {
				$names = [];
				$sql = "SELECT first, last from ".$wpdb->prefix."rsvpmaker WHERE event=$post_id ORDER BY first, last";
				$rsvps = $wpdb->get_results($sql);
				if($rsvps) {
					foreach($rsvps as $rsvp) {
						$display_name = $rsvp->first.' '.$rsvp->last;
						if(!in_array($display_name,$memberlist))
							$memberlist[] = array('label'=>$display_name,'value'=>$display_name.' (guest)');
					}
				}
		...
		$voteresult = wptm_count_votes($post_id, $votingdata);
			...
			if ( is_array( $value ) ) {
				if ( $name == '_role_Speaker' ) {
					...
					foreach ( $value as $assigned ) {
							$neworder[] = wp4t_get_speaker_array( sanitize_text_field($assigned), $post_id );
					}
					foreach ( $neworder as $index => $speaker ) {
						wp4t_save_speaker_array( $speaker, $index + 1, $post_id );
						$assigned = $speaker['ID'];
						if ( empty( $assigned ) ) {
							$assigned = '?';
...
class Editable_Note_Json extends WP_REST_Controller {
	...
	public function get_items_permissions_check( $request ) {
		return true;// (!empty($_POST)) ? wp4t_is_club_member() : true;
	}
	public function handle( $request ) {
	rsvpmaker_debug_log($_SERVER['SERVER_NAME'].' '.$_SERVER['REQUEST_URI'],'rsvpmaker_api');
		global $wpdb, $current_user;
...
function wpt_get_agendadata($post_id = 0, $render = true) {
	...
	foreach($absences as $ab) {
		...
		$agendadata['permissions'] = array('member' => wp4t_is_club_member(), 'edit_post' => current_user_can('edit_post',$post_id),'edit_signups' => current_user_can('edit_signups'),'organize_agenda'=>current_user_can('organize_agenda'),'manage_options' => current_user_can('manage_options'));
		$agendadata['allmembers'] = wp4t_awe_rest_user_options('',$post_id);
...
class WP4T_Mobile_Agenda extends WP_REST_Controller {
	...
	rsvpmaker_debug_log($_SERVER['SERVER_NAME'].' '.$_SERVER['REQUEST_URI'],'rsvpmaker_api');
		...
		{
			delete_transient('wp4t_translations');
		...
		if(empty($memberlist) || !is_array($memberlist)) {
			...
			if(!empty($agendadata['agendas']))
				{
					foreach($agendadata['agendas'] as $agenda) {
						if($agenda['post_id'] == $post_id && !empty($agenda['guests'])) {
							foreach($agenda['guests'] as $guest) {
								$memberlist[] = array('name'=>$guest,'ID'=>$guest);
							}
						}
					}
				}
			$sql = $wpdb->prepare("SELECT first, last from {$wpdb->prefix}rsvpmaker WHERE event=%d ORDER BY first, last", $post_id);
			$rsvps = $wpdb->get_results($sql);
			if($rsvps) {
				if(empty($names)) $names = array();
				foreach($rsvps as $rsvp) {
					$display_name = $rsvp->first.' '.$rsvp->last.' (guest)';
					if(!in_array($display_name,$names)) {
						$names[] = $display_name;
						$memberlist[] = array('name'=>$display_name,'ID'=>$display_name);
					}
				}
			}
			set_transient('wp4t_memberlist',$memberlist,HOUR_IN_SECONDS);
		}
...
/**
 * Clear the cached blocks data whenever an rsvpmaker post is updated.
 *
 * @param int     $post_id Post ID.
 * @param WP_Post $post    Post object.
 * @param bool    $update  Whether this is an existing post being updated.
 */
function wp4t_clear_blocksdata_cache( $post_id, $post, $update ) {
    // Only clear the cache if this is an actual update to an existing event
    if ( ! $update ) {
        return;
    }

    // Delete the cached meta key so it regenerates on the next API call
    delete_post_meta( $post_id, '_blocksdata' );
}

add_action( 'save_post_rsvpmaker', 'wp4t_clear_blocksdata_cache', 10, 3 );
function wpt_get_mobile_agendadata($user_id = 0) {
    ...
    foreach($meetings as $meeting) {
        ...
        // Fetch all custom fields for this meeting in ONE query
        $all_meta = get_post_custom($post_id);

        // Helper closures to quickly read from our single database hit cache
        $get_cached_meta_single = function($key) use ($all_meta) {
            return (isset($all_meta[$key][0])) ? $all_meta[$key][0] : '';
        };
        $get_cached_meta_array = function($key) use ($all_meta) {
            return (isset($all_meta[$key])) ? $all_meta[$key] : [];
        };

        // Pull cached block data or parse and update it
        $blocksdata_serialized = $get_cached_meta_single('_blocksdata');
        $blocksdata = !empty($blocksdata_serialized) ? maybe_unserialize($blocksdata_serialized) : [];
        ...
        foreach($blocksdata as $block) {
            if(isset($block['attrs']) && isset($block['attrs']['role'])) {
                ...
                for($i = $start; $i < ($count + $start); $i++) {
                    ...
                    $assignment['ID'] = $get_cached_meta_single($key);
                    $assignment['suggestion'] = '';
                    
                    if(empty($assignment['ID'])) {
                        $assignment['name'] = '';
                        // Optimized: Reading from internal array
                        $assignment['suggestion'] = $get_cached_meta_single('_suggest' . $key);
                    }
                    elseif(is_numeric($assignment['ID'])) {
                        $assignment['name'] = wp4t_get_member_name($assignment['ID']);
                    }
		...
		$agenda['guests'] = $guests;
		$guests = []; // Reset for the next agenda
        ...
        if(strpos($meeting->post_content, 'wp:wp4toastmasters/absences')) {
            ...
            // Optimized: Reading multi-row data cleanly from cache wrapper
            $absences = $get_cached_meta_array('wp4t_tm_absence');
...
class WP4TUpdateAgenda extends WP_REST_Controller {
	...
	rsvpmaker_debug_log($_SERVER['SERVER_NAME'].' '.$_SERVER['REQUEST_URI'],'rsvpmaker_api');
		...
		if(!empty($data->copyToTemplate)) {
			...
			$related = rsvpmaker_get_events_by_template($data->has_template);
...
class WP4T_Members_for_Role extends WP_REST_Controller {
	...
	public function handle( $request ) {
	rsvpmaker_debug_log($_SERVER['SERVER_NAME'].' '.$_SERVER['REQUEST_URI'],'rsvpmaker_api');
		$options = wp4t_awe_rest_user_options( wp4t_clean_role($request['role']), $request['post_id'] );
		...
		foreach($item as $type => $value) {
			...
			if(($type == "ID") && $wasopen && !empty($value)) {
				...
				if(!empty($was)) {
					$agendadata = wpt_get_agendadata($post_id);
					$agendadata['taken'] = wp4t_get_member_name($was);
					$agendadata['status'] = 'already assigned: '.wp4t_get_member_name($was);
...
class WP4T_Absences extends WP_REST_Controller {
	...
	public function get_items_permissions_check( $request ) {
		return true;//return wp4t_is_club_member();
	}
	public function handle( $request ) {
	rsvpmaker_debug_log($_SERVER['SERVER_NAME'].' '.$_SERVER['REQUEST_URI'],'rsvpmaker_api');
		global $rsvp_options;
...
class WP4T_Hybrid extends WP_REST_Controller {
	...
	rsvpmaker_debug_log($_SERVER['SERVER_NAME'].' '.$_SERVER['REQUEST_URI'],'rsvpmaker_api');
		...
		$response = wp4t_tm_attend_in_person_json($data);
		return new WP_REST_Response($response,
			200
		);
	}
...
class WP4T_Evaluation extends WP_REST_Controller {
	...
	rsvpmaker_debug_log($_SERVER['SERVER_NAME'].' '.$_SERVER['REQUEST_URI'],'rsvpmaker_api');
		...
		if(!empty($data)) {
			...
			$project_text = ($data->evaluate->project) ? wp4t_get_project_text($data->evaluate->project) : '';
			...
			foreach($prompts as $index => $p) {
				...
				if($p->choices) {
					...
					foreach($p->choices as $option) {
						$output .= wp4t_format_eval_opt($option->value,($choice == $option->value));
					}
					$output .= "</div>\n";
				}
	...
	'meta_value' => $response,
		...
		if(!empty(trim($project)))
			$form    = wp4t_fetch_evaluation_form( $project );
		if(empty($form) || empty($form->intro) ) {
			$response['project'] = 'Dynamic Leadership Level 1 Mastering Fundamentals 5';
			$form = wp4t_fetch_evaluation_form( 'Dynamic Leadership Level 1 Mastering Fundamentals 5' );
		...
		foreach($p as $speech) {
			$speech->name = $label = wp4t_get_member_name($speech->ID);
...
function wp4t_format_eval_opt($opt,$is_true) {
