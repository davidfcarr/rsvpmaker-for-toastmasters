...
add_action( 'admin_init', 'toastmasters_fix_agenda_attribute_type' );



function toastmasters_fix_agenda_attribute_type() {



	global $wpdb;

	$version = get_option( 'toastmasters_agenda_attribute_version', 0 );
	...
	$sql = "SELECT ID, post_content FROM $wpdb->posts WHERE post_content LIKE '%\"count\":\"%' OR post_content LIKE '%\"time_allowed\":\"%' OR post_content LIKE '%\"padding_time\":\"%'";
	...
	if ( empty( $posts ) ) {
		...
		// Nothing to do — still mark as complete so we don't check again.

		error_log( 'toastmasters_fix_agenda_attribute_type: no posts found needing update.' );
	...
	foreach ( $posts as $index => $post ) {
		...
		// Replace  "count":"<digits>"  with  "count":<digits>
		...
		$new_content = preg_replace(
			...
			'/"(count|time_allowed|padding_time)"\s*:\s*"(\d+)"/',
		...
		if ( $new_content !== $original_content ) {
			...
			// Bust the post cache so stale content isn't served.
	...
	// Log result to the error log for easy verification.
	...
	error_log(
		...
		sprintf(
			...
			'toastmasters_fix_agenda_attribute_type: migration complete. %d of %d posts updated.',
...
function wpt_public_profile($id,$contactmethods,$attributes, $is_officer = false) {
	...
	if(!empty($data->tm_directory_blocked) || get_user_meta($id,'tm_directory_blocked',true))
