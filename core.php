...
<ul>
	...
	echo '</ul>';
	if(!empty($submenu['toastmasters_admin_help']))
...
function wp4toast_setup() {
	...
	if ( empty( $setup ) ) {
		...
		if ( $wpdb->get_var( "SELECT post_title from $wpdb->posts WHERE post_type='page' AND post_status='publish' AND ( post_content LIKE '%[awesome_members%' OR post_content LIKE '%[wp4t_awesome_members%' )" ) ) {
...
function wp4t_decode_editable_note_content( $content ) {
	...
	$decode_unicode = static function ( $matches ) {
		$hex = isset( $matches[1] ) ? $matches[1] : '';
		if ( ! preg_match( '/^[0-9a-fA-F]{4}$/', $hex ) ) {
			return $matches[0];
		}

		$decoded = html_entity_decode( '&#x' . strtolower( $hex ) . ';', ENT_QUOTES | ENT_HTML5, 'UTF-8' );
		return ( '' === $decoded ) ? $matches[0] : $decoded;
	};

	// Decode both JSON-style escapes (\u00e9) and broken bare escapes (u00e9).
	$content = preg_replace_callback( '/\\\\+u([0-9a-fA-F]{4})/', $decode_unicode, $content );
	$content = preg_replace_callback( '/u([0-9a-fA-F]{4})/', $decode_unicode, $content );
...
function wp4t_agenda_note( $atts, $content = '' ) {
	...
	if ( ! empty( $atts['editable'] ) || isset( $atts['uid'] ) || ! empty( $atts['defaultContent'] ) ) {
		...
		if ( is_single() && wp4t_is_club_member() && ! isset( $_REQUEST['edit_roles'] ) && ! isset( $_REQUEST['recommend_roles'] ) && ( current_user_can( 'edit_signups' ) || wp4t_edit_signups_role() ) && ! isset( $_REQUEST['print_agenda'] )  && ! isset( $_REQUEST['show_agenda'] ) && ! rsvpmaker_is_email_context() ) {
	...
	if ( isset( $_REQUEST['print_agenda'] ) || isset( $_REQUEST['show_agenda'] ) || isset( $_REQUEST['email_agenda'] ) ) {
...
function toastmasters_agenda_display( $atts, $assignments ) {
	...
	foreach ( $assignments as $field => $values ) {
		...
		if ( isset( $atts['wp4t_agenda_note'] ) && ! empty( $atts['wp4t_agenda_note'] ) && strpos( $atts['wp4t_agenda_note'], 'Speaker}' ) ) {
			...
			$decoded_note_template = wp4t_decode_editable_note_content( $atts['wp4t_agenda_note'] );

			$speakerID_to_be_evaluated = get_post_meta( $post->ID, '_role_Speaker_' . $i, true );

			if ( $speakerID_to_be_evaluated > 0 ) {

				$note = str_replace( '{Speaker}', wp4t_get_member_name( $speaker_to_be_evaluated ), $decoded_note_template );

			} else {

				$note = str_replace( '{Speaker}', '?', $decoded_note_template );

			}

			$output .= '<div class="role_agenda_note">' . esc_html( $note ) . '</div>';

		}

		

		if(isset($atts['blockName']) && 'wp4toastmasters/context' == $atts['blockName']) {
	...
	if ( isset( $atts['wp4t_agenda_note'] ) && ! empty( $atts['wp4t_agenda_note'] ) && ! strpos( $atts['wp4t_agenda_note'], 'Speaker}' ) ) {
		...
		$note    = wp4t_decode_editable_note_content( $atts['wp4t_agenda_note'] );
...
function toastmaster_short( $atts = array(), $content = '' ) {
	...
	if ( ( isset( $_GET['print_agenda'] ) || isset( $_GET['show_agenda'] ) ) && strpos( $field_base, 'ackup' ) && ( $assigned < 1 ) ) {
	...
	if ( ! wp4t_is_edit_roles() && ( ( isset( $_REQUEST['print_agenda'] ) || isset( $_REQUEST['show_agenda'] ) || rsvpmaker_is_email_context() || wp4t_is_agenda_locked() ) ) ) {
		...
		if ( wp4t_is_club_member() ) {
			...
			if ( wp4t_is_edit_roles() && ( current_user_can( 'edit_signups' ) || wp4t_edit_signups_role() ) ) {
				...
				if ( rsvpmaker_is_template() ) {
					...
					if ( ( $points < 0 ) && ! empty( $rules['points'] ) ) {
						...
						if ( $rules['points'] == 'prevent' ) {
							...
							if ( wp4t_has_tm_attend_in_person_shortcode( $post->post_content ) )
				...
				if ( strpos( $field, 'Speaker' ) ) {
					...
					$in_person = wp4t_has_tm_attend_in_person_shortcode( $post->post_content ) ? wp4t_tm_in_person_checkbox() : '';
	...
	foreach($blocksdata as $blockindex => $block) {
		...
		if('wp4toastmasters/role' == $block["blockName"] && !empty($attrs['role'])) {
...

			{
				...
				$content .= sprintf('<div class="eachrole">%s <strong>%s %s</strong> %s',$avatar,$role,($count > 1) ? $number : '',$assignment['name']);
				...
				if(('Speaker' == $role) && $number) {
					...
					if((rsvpmaker_is_email_context() || isset($_GET['show_agenda'])) && !empty($assignment['project']))
...
function wp4t_evaluation_form_link( $speaker, $meeting_id, $project_key = '' ) {
	...
	if ( isset( $_GET['print_agenda'] ) ||  isset( $_GET['show_agenda'] ) ) {
	...
	if ( ! isset( $_GET['print_agenda'] ) && ! isset( $_GET['show_agenda'] ) ) {
		...
		return sprintf( ' (<a href="%s" target="_blank">%s</a>)', wp4t_evaluation_form_url($speaker, $meeting_id), __( 'evaluation form', 'rsvpmaker-for-toastmasters' ) );
...
function wp4toast_reminders_intros($post_id = 0, $hours = 0) {
...
if(!$next || ($next->ts_start > time() + HOUR_IN_SECONDS))

	return;

		$date = rsvpmaker_date($rsvp_options['long_date'], (int) $next->ts_start);

		$msg = '';

		echo $sql = "SELECT * FROM $wpdb->postmeta WHERE post_id=$next->ID AND meta_key LIKE '_role_Speaker%' AND meta_value > 0 ORDER BY meta_key";

		$results = $wpdb->get_results($sql);

		error_log($sql.var_export($results,true));
...
$formbottom = wp4t_has_tm_attend_in_person_shortcode( $post->post_content ) ? wp4t_tm_in_person_checkbox($user_id) : '';
...
$output .= sprintf('<p>%s signed up for %s. <a href="%s">View agenda</a></p>',$user->display_name,$clean,add_query_arg(array('show_agenda' =>1),get_permalink()) );
...
function dash_wptmagenda_menu( $post_id, $rsvp_report = '' ) {
	...
	$link     .= '<li ><a target="_blank" href="' . $permalink . 'show_agenda=1">' . __( 'Agenda Show', 'rsvpmaker-for-toastmasters' ) . '</a></li>';

	if ( ! get_option( 'wp4toastmasters_intros_on_agenda' ) ) {

		$link .= '<li class="last"><a target="_blank" href="' . $permalink . 'show_agenda=1&showintros=1">' . __( 'Show with Introductions', 'rsvpmaker-for-toastmasters' ) . '</a></li>';
	...
	$link    .= '<li ><a target="_blank" href="' . add_query_arg('mvote',1,$permalink) . '">' . __( 'Vote Counter\'s Tool', 'rsvpmaker-for-toastmasters' ) . '</a></li>';
...
function wp4t_awesome_user_profile_fields( $user ) {
	...
	$public_email   = get_user_meta( $user->ID, 'public_email', true );
...
<th><label for="public_profile"><?php _e( 'Profile Visibility', 'rsvpmaker-for-toastmasters' ); ?></label></th>

<td>
<?php
printf('<p>Should your contact information be shared with other members? <br /><input type="radio" name="tm_directory_blocked" value="0" %s /> %s ',
	( ! $profile_blocked ) ? 'checked="checked"' : '',
	__( 'Yes', 'rsvpmaker-for-toastmasters' )
);
printf('<input type="radio" name="tm_directory_blocked" value="1" %s /> %s</p>',
	( $profile_blocked ) ? 'checked="checked"' : '',
	__( 'No', 'rsvpmaker-for-toastmasters' )
);
if(!$profile_blocked) {
	printf('<p>Should your name, picture, and bio be shown in the club\'s public directory? <br /><input type="radio" name="public_profile" value="1" %s /> %s ',
		( $public_profile ) ? 'checked="checked"' : '',
		__( 'Yes', 'rsvpmaker-for-toastmasters' )
	);
	printf('<input type="radio" name="public_profile" value="0" %s /> %s</p>',
		( !$public_profile ) ? 'checked="checked"' : '',
		__( 'No', 'rsvpmaker-for-toastmasters' )
	);
	printf('<p>Should your email be shown in the club\'s public directory? <br /><input type="radio" name="public_email" value="1" %s /> %s ',
		( $public_email ) ? 'checked="checked"' : '',
		__( 'Yes', 'rsvpmaker-for-toastmasters' )
	);
	printf('<input type="radio" name="public_email" value="0" %s /> %s</p>',
		( !$public_email ) ? 'checked="checked"' : '',
		__( 'No', 'rsvpmaker-for-toastmasters' )
	);
	printf('<p>Should your phone number be shown in the club\'s public directory? <br /><input type="radio" name="public_phone" value="1" %s /> %s ',
		( $public_phone ) ? 'checked="checked"' : '',
		__( 'Yes', 'rsvpmaker-for-toastmasters' )
	);
	printf('<input type="radio" name="public_phone" value="0" %s /> %s</p>',
		( !$public_phone ) ? 'checked="checked"' : '',
		__( 'No', 'rsvpmaker-for-toastmasters' )
	);
}	
...
function wp4t_save_awesome_user_profile_fields( $user_id ) {
	...
	update_user_meta( $user_id,'second_language_feedback',!empty($_POST['second_language_feedback']) ? 1 : 0 );

	//update_user_meta( $user_id,'wp4t_tm_privacy_prompt',intval($_POST['tm_privacy_permission']));

	update_user_meta( $user_id,'tm_directory_blocked',!empty($_POST['tm_directory_blocked']) ? 1 : 0 );
	...
	//update_user_meta( $user_id, 'public_phone', ! empty( $_POST['public_phone'] ) );
...
<head>
<title>Speech Introductions <?php wp_title( '|', true, 'right' ); ?></title>
<style>
...
function wp4t_redirect() {
		...
		elseif ( isset( $_REQUEST['print_agenda'] ) || isset( $_REQUEST['show_agenda'] ) ) {
...
function toast_modify_query_exclude_category( $query ) {
	...
	if ( $query->get( 'post_type' ) === 'rsvpmaker' || is_singular( 'rsvpmaker' ) || is_post_type_archive( 'rsvpmaker' ) ) {
		$query->set(
			'tax_query',
			array(
				array(
...
function rsvptoast_admin_notice() {
	...
	if ( isset( $_REQUEST['page'] ) && ( $_REQUEST['page'] == 'agenda_timing' ) && isset( $_REQUEST['post_id'] ) ) {
		...
		printf( '<a href="%s" target="_blank">%s</a> | ', rsvpmaker_permalink_query( (int) $_REQUEST['post_id'], 'show_agenda=1' ), __( 'View Agenda', 'rsvpmaker-for-toastmasters' ) );
...
function wp4t_tm_security_setup( $check = true, $cookie = true ) {
	...
	if ( $check ) {
		...
		if ( ! empty( $security ) ) {
			...
			foreach($security as $role => $values)

				$tm_security[$role] = $values;
...
<h2>Toastmasters Security Options</h2>
	...
	if ( isset( $_POST['tm_caps'] ) && $_POST['tm_caps'] && wp_verify_nonce(rsvpmaker_nonce_data('data'),rsvpmaker_nonce_data('key')) ) {
		...
		update_option('tm_caps',$_POST['tm_caps']);
	...
	settings_fields( 'wp4toastmasters-security' );
...
add_shortcode( 'blogs_by_member_tag', 'wp4t_blogs_by_member_tag' );
...
function wp4t_tm_absence( $atts ) {
	...
	if ( isset( $atts['show_on_agenda'] ) && empty( $atts['show_on_agenda'] ) && ( isset( $_GET['print_agenda'] ) || isset( $_GET['email_agenda'] ) ) ) {
...
function wpt_embed_agenda( $atts ) {
	...
	return sprintf( '<iframe src="%sshow_agenda=1" style="%s"></iframe>', $permalink, $style );
...
if ( ! wp_is_json_request() ) {
	...
	$wp4t_shortcodes = array(
		'wpt_open_roles'        => 'wpt_open_roles',
		'contest_demo'          => 'wp4t_contest_demo',
		'agenda_role'           => 'toastmaster_short',
		'toastmaster'           => 'toastmaster_short',
		'agenda_note'           => 'wp4t_agenda_note',
		'officer'               => 'toastmasters_officer_single',
		'toastmaster_officers'  => 'toastmaster_officers',
		'stoplight'             => 'wp4t_stoplight_shortcode',
		'awesome_members'       => 'wp4t_awesome_members',
		'signup_sheet'          => 'wp4t_signup_sheet',
		'themewords'            => 'wp4t_themewords',
		'jstest'                => 'wp4t_jstest',
		'club_news'             => 'wp4t_club_news',
		'members_only'          => 'wp4t_members_only',
		'tmlayout_club_name'    => 'wp4t_tmlayout_club_name',
		'tmlayout_tag_line'     => 'wp4t_tmlayout_tag_line',
		'tmlayout_post_title'   => 'wp4t_tmlayout_post_title',
		'tmlayout_meeting_date' => 'wp4t_tmlayout_meeting_date',
		'tmlayout_sidebar'      => 'wp4t_tmlayout_sidebar',
		'tmlayout_main'         => 'wp4t_tmlayout_main',
		'tmlayout_intros'       => 'wp4t_speech_intros_shortcode',
		'wpt_officers'          => 'wpt_officers',
		'speaker_evaluator'     => 'wp4t_speaker_evaluator',
		'evaluation_links'      => 'wp4t_evaluation_links',
		'wpt_speakers'          => 'wpt_speakers',
		'wpt_evaluators'        => 'wpt_evaluators',
		'wpt_general_evaluator' => 'wpt_general_evaluator',
		'wpt_tod'               => 'wpt_tod',
		'wptagendalink'         => 'wptagendalink',
		'wp4t_assigned_open'    => 'wp4t_assigned_open',
		'tm_absence'            => 'wp4t_tm_absence',
		'wpt_embed_agenda'      => 'wpt_embed_agenda',
		'tm_branded_image'      => 'wp4t_tm_branded_image',
		'tm_member_application' => 'wp4t_tm_member_application',
	);

	foreach ( $wp4t_shortcodes as $tag => $callback ) {
		add_shortcode( $tag, $callback );
	}

	$wp4t_shortcode_aliases = array(
		'wp4t_contest_demo'          => 'wp4t_contest_demo',
		'wp4t_agenda_note'           => 'wp4t_agenda_note',
		'wp4t_awesome_members'       => 'wp4t_awesome_members',
		'wp4t_signup_sheet'          => 'wp4t_signup_sheet',
		'wp4t_themewords'            => 'wp4t_themewords',
		'wp4t_jstest'                => 'wp4t_jstest',
		'wp4t_club_news'             => 'wp4t_club_news',
		'wp4t_members_only'          => 'wp4t_members_only',
		'wp4t_tmlayout_club_name'    => 'wp4t_tmlayout_club_name',
		'wp4t_tmlayout_tag_line'     => 'wp4t_tmlayout_tag_line',
		'wp4t_tmlayout_post_title'   => 'wp4t_tmlayout_post_title',
		'wp4t_tmlayout_meeting_date' => 'wp4t_tmlayout_meeting_date',
		'wp4t_tmlayout_sidebar'      => 'wp4t_tmlayout_sidebar',
		'wp4t_tmlayout_main'         => 'wp4t_tmlayout_main',
		'wp4t_speaker_evaluator'     => 'wp4t_speaker_evaluator',
		'wp4t_evaluation_links'      => 'wp4t_evaluation_links',
		'wp4t_tm_absence'            => 'wp4t_tm_absence',
		'wp4t_tm_branded_image'      => 'wp4t_tm_branded_image',
		'wp4t_tm_member_application' => 'wp4t_tm_member_application',
	);

	foreach ( $wp4t_shortcode_aliases as $tag => $callback ) {
		add_shortcode( $tag, $callback );
	}
...
add_shortcode('tmlayout_role_iterator','wp4t_tmlayout_role_iterator');

add_shortcode('wp4t_tmlayout_role_iterator','wp4t_tmlayout_role_iterator');

add_shortcode('tmlayout_role_output','wp4t_tmlayout_role_output');

add_shortcode('wp4t_tmlayout_role_output','wp4t_tmlayout_role_output');

function wp4t_has_tm_attend_in_person_shortcode( $content ) {
	return (
		false !== strpos( $content, 'tm_attend_in_person' )
		|| false !== strpos( $content, 'wp4t_tm_attend_in_person' )
		|| false !== strpos( $content, 'wp4toastmasters/hybrid' )
	);
}
...
add_shortcode('tm_attend_in_person','wp4t_tm_attend_in_person');
...
add_shortcode('rsvp_attend_in_person','wp4t_rsvp_attend_in_person');
