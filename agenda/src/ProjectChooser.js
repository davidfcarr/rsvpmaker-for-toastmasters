...
export default function ProjectChooser(props) {
    ...
    return (
        ...
        <div><SelectCtrl options={choices['paths']} value={path} label="Path" onChange={(value) => {setPath(value); updateSpeech();}} /></div>

        <div><SelectCtrl options={choices['manuals'][path]} value={manual} label="Level" onChange={(value) => {setManual(value); updateSpeech();}} /></div>
        ...
        <TextControl label="Display Time" onChange={(value) => { setDisplayTime(value); updateSpeech(); } } value={display_time} />

        </div>

        <div className="tmflex50">

        <TextControl label="Maximum Time Allowed" onChange={(value) => { setMaxTime(value); updateSpeech(); }} value={maxtime} />

        </div>

        </div>

        <p><strong>Title</strong> <TextControl value={title} onChange={(value) => {setTitle(value); }} onMouseLeave={() => updateSpeech()} /></p>
