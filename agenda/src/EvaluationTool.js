...
export default function EvaluationTool(props) {
    ...
    if (isLoading || isFetching)
        return <p>{mytranslate('Loading ...', data)}</p>;
    ...
    return (
        <div className="eval">
            <h2>{mytranslate('Evaluation Tool', data)}</h2>
            {sent && (
                <div>
                    <p>
                        <button onClick={copyEvaluation}>
                            {mytranslate('Copy to Clipboard', data)}
                        </button>{' '}
                        <em>{mytranslate('works in most browsers, but not Firefox', data)}</em>
                    </p>
                    <p>
                        <a href={resetLink}>{mytranslate('Reset', data)}</a>
                    </p>
                    <SanitizedHTML innerHTML={sent} />
                    <p>
                        <a href={resetLink}>{mytranslate('Reset', data)}</a>
                    </p>
                </div>
            ...
            <p><button className="tmform" onClick={send}>{mytranslate('Send', data)}</button></p>
        </div>
    );
}
