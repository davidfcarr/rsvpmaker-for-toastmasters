...
foreach($speaker_assignments as $index => $sa) {
	$ea = isset($evaluator_assignments[$index]) ? $evaluator_assignments[$index] : array();
	// Some rows are partial (for example backups) and can omit keys like manual/title.
	$sa_name = isset($sa['name']) ? $sa['name'] : '';
	$sa_manual = isset($sa['manual']) ? $sa['manual'] : '';
	$sa_title = isset($sa['title']) ? $sa['title'] : '';
	$ea_name = isset($ea['name']) ? $ea['name'] : '';
	if(5 == $columns) {
		...
		printf('<tr><td>%d</td><td>%s</td><td><strong>%s</strong></td></tr>',$index+1,$sa_text,$ea_name);	
	}
}
?>
</table>
