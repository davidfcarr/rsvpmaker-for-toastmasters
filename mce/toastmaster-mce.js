(function() {
	tinymce.PluginManager.add('toastmaster', function( editor, url ) {
		...
		function replaceShortcodes( content ) {
			return content.replace( /\[toastmaster([^\]]*)\]/g, function( all,attr) {
		...
		//add popup
		editor.addCommand('toastmaster_popup', function(ui, v) {
			if (v.themewords)
				return;
			//setup defaults
			var count = '1';
		...
		//add button
		editor.addButton('toastmaster', {
			icon: 'toastmaster',
			tooltip: 'Toastmaster Role',
			onclick: function() {
				editor.execCommand('toastmaster_popup','',{
					count   : '1',
					role: ''
				});
			}
